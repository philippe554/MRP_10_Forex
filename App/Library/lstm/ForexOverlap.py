...
class ForexOverlap(ForexBase):
    ...
    def get_X_test(self, batch_size):
        ...
        for batch in range(self.batch_size):
            if self.test_offset + self.sequence_size + self.sequence_overlap >= self.test_size:
                self.test_offset += self.sequence_size + self.sequence_overlap
                price[batch, :] = self.price_test[-1, 0]
                continue

            X[batch, :, :] = self.TA_test[
                             self.test_offset: (self.test_offset + (self.sequence_size + self.sequence_overlap)), :]
            price[batch, :] = self.price_test[
                              self.test_offset: (self.test_offset + (self.sequence_size + self.sequence_overlap)), 0]
